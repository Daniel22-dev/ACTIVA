...
fs.rmSync(path.join(root, 'qa-results', 'current'), { recursive: true, force: true });
...
const repository = process.env.GHRAB_SOURCE_REPOSITORY || process.env.GITHUB_REPOSITORY || 'Daniel22-dev/ACTIVA';
...
for (const sourceDir of [path.join(root, 'dist'), path.join(root, 'test-results')]) {
  ...
  for (const name of fs.readdirSync(sourceDir)) {
    ...
    if (/^(?:qa-|quality-report|garp-).*\.json$/i.test(name) || name.endsWith('-report.json')) fs.copyFileSync(file, path.join(qaResults, `${path.basename(sourceDir)}-${name}`));
  }
}

const selftestEnv = { ...process.env }; delete selftestEnv.GHRAB_SOURCE_COMMIT; delete selftestEnv.GITHUB_SHA;
const selftest = spawnSync(process.execPath, [path.join(root, 'security', 'garp25', 'tools', 'selftest-garp251.mjs')], { cwd: root, encoding: 'utf8', env: selftestEnv });
...
if (selftest.status !== 0) throw new Error(`Release identity FAIL: GARP selftest failed. ${selftest.stderr || ''}`);

for (const [name, script] of [['ai-studio-dispatch-contract.txt','scripts/test-ai-studio-dispatch.mjs'], ['studio-manifest-contract.txt','scripts/verify-studio-manifest-contract.mjs'], ['auto-patch-topology.txt','scripts/qa-auto-patch-notification.mjs']]) {
  const args = script.includes('studio-manifest') ? [studioManifestPath] : [];
  const test = spawnSync(process.execPath, [path.join(root, script), ...args], { cwd: root, encoding: 'utf8' });
  fs.writeFileSync(path.join(qaResults, name), `${test.stdout || ''}${test.stderr || ''}`);
  if (test.status !== 0) throw new Error(`Release identity FAIL: ${script} failed.`);
}

runNode('scripts/stamp-live-release-identity.mjs', [studioManifestPath], { GHRAB_SOURCE_REPOSITORY: repository, GHRAB_SOURCE_COMMIT: source, GHRAB_BUILD_ID: buildId });
runNode('scripts/verify-studio-manifest-contract.mjs', [studioManifestPath]);
runNode('scripts/generate-cyclonedx-sbom.mjs', [sbomPath, dist, '--deploy']);
runNode('security/garp25/tools/create-evidence-manifest.mjs', [qaResults, evidencePath], { GHRAB_APP_ID: appId, GHRAB_APP_VERSION: version, GHRAB_SOURCE_COMMIT: source });
...
runNode('scripts/verify-live-release-identity.mjs', [integrityPath], { GHRAB_SOURCE_REPOSITORY: repository, GHRAB_SOURCE_COMMIT: source });
runNode('security/garp25/tools/scan-deployment-leaks.mjs', [dist]);
for (const field of ['artifactDigest', 'manifestSha256', 'sbomSha256', 'buildProvenanceSha256', 'evidenceManifestSha256']) if (!/^[0-9a-f]{64}$/i.test(String(integrity[field] || ''))) throw new Error(`Release identity FAIL: ${field} is not SHA-256.`);
