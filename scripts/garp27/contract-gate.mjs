...
results.push(run('activity-builder-policy',[path.join(V,'MASTER/TOOLS/validate-policy.mjs'),path.join(ROOT,'security/garp27/garp-policy.json'),'--core',path.join(V,'MASTER/CONTRACTS/garp27-core.json'),'--inventory',path.join(V,'MASTER/INVENTORY/ecosystem-apps.json')]));
...
const failed=results.filter(x=>!x.pass).length;console.log(JSON.stringify({classification:'GARP27_CONTRACT_GATE',status:failed?'FAIL':'PASS',garpVersion:'2.7',appId:'activity-builder',serverPhase:'DEFERRED_BY_OWNER_DECISION',results,summary:{total:results.length,passed:results.length-failed,failed}},null,2));process.exit(failed?1:0);
