...
function makeStoredZip(entries){
  ...
  for(const [name,value] of entries){
    const n=Buffer.from(name,'utf8'), data=Buffer.isBuffer(value)?value:Buffer.from(String(value),'utf8');
    const local=Buffer.alloc(30+n.length);local.writeUInt32LE(0x04034b50,0);local.writeUInt16LE(20,4);local.writeUInt16LE(0,6);local.writeUInt16LE(0,8);local.writeUInt16LE(0,10);local.writeUInt16LE(0,12);local.writeUInt32LE(0,14);local.writeUInt32LE(data.length,18);local.writeUInt32LE(data.length,22);local.writeUInt16LE(n.length,26);local.writeUInt16LE(0,28);n.copy(local,30);
    locals.push(local,data);
    const c=Buffer.alloc(46+n.length);c.writeUInt32LE(0x02014b50,0);c.writeUInt16LE(20,4);c.writeUInt16LE(20,6);c.writeUInt16LE(0,8);c.writeUInt16LE(0,10);c.writeUInt16LE(0,12);c.writeUInt16LE(0,14);c.writeUInt32LE(0,16);c.writeUInt32LE(data.length,20);c.writeUInt32LE(data.length,24);c.writeUInt16LE(n.length,28);c.writeUInt16LE(0,30);c.writeUInt16LE(0,32);c.writeUInt16LE(0,34);c.writeUInt16LE(0,36);c.writeUInt32LE(0,38);c.writeUInt32LE(offset,42);n.copy(c,46);centrals.push(c);offset+=local.length+data.length;
  }
  const central=Buffer.concat(centrals), body=Buffer.concat(locals);const e=Buffer.alloc(22);e.writeUInt32LE(0x06054b50,0);e.writeUInt16LE(0,4);e.writeUInt16LE(0,6);e.writeUInt16LE(entries.length,8);e.writeUInt16LE(entries.length,10);e.writeUInt32LE(central.length,12);e.writeUInt32LE(body.length,16);e.writeUInt16LE(0,20);return Buffer.concat([body,central,e]);
}
...
try {
  ...
  await writeFile(path.join(leak, 'bundle.js'), '// b\n' + 'a'.repeat(2_100_000) + '\nconst K="' + 'AIza' + 'SyFAKEFAKEFAKEFAKEFAKEFAKEFAKE00' + '";\n');
  r = run('scan-deployment-leaks.mjs', [leak]);
  ...
  // LU-N18/INFO: canonical negative controls for binary, CSS/SVG, OOXML and ordinary/nested ZIP.
  await writeFile(path.join(leak, 'binary.png'), Buffer.concat([Buffer.from([0x89,0x50,0x4e,0x47,0x00,0xff]),Buffer.from('AIza'+'BinarySyntheticKey123456789012345')]));
  r = run('scan-deployment-leaks.mjs', [leak]);
  expect('NC-secret-in-binary-bytes-detected', r.status !== 0);
  await rm(path.join(leak, 'binary.png'));
  await writeFile(path.join(leak, 'style.css'), 'body{--x:"sk-'+'SyntheticCssSecret123456789012345";}');
  r = run('scan-deployment-leaks.mjs', [leak]); expect('NC-secret-in-css-detected', r.status !== 0); await rm(path.join(leak, 'style.css'));
  await writeFile(path.join(leak, 'vector.svg'), '<svg><text>github_pat_'+'SyntheticSvgSecret123456789012345</text></svg>');
  r = run('scan-deployment-leaks.mjs', [leak]); expect('NC-secret-in-svg-detected', r.status !== 0); await rm(path.join(leak, 'vector.svg'));
  await writeFile(path.join(leak, 'fixture.docx'), makeStoredZip([['word/document.xml','<w:t>AIza'+'OoxmlSyntheticKey123456789012345</w:t>']]));
  r = run('scan-deployment-leaks.mjs', [leak]); expect('NC-secret-in-ooxml-detected', r.status !== 0); await rm(path.join(leak, 'fixture.docx'));
  const innerZip=makeStoredZip([['secret.txt','sk-'+'NestedZipSyntheticSecret123456789012345']]);
  await writeFile(path.join(leak, 'archive.zip'), makeStoredZip([['nested.zip',innerZip]]));
  r = run('scan-deployment-leaks.mjs', [leak]); expect('NC-secret-in-general-nested-zip-detected', r.status !== 0); await rm(path.join(leak, 'archive.zip'));
...
const CACHE='x-v2'; const CACHE_PREFIXES=['x-v'];
self.addEventListener('activate',e=>e.waitUntil((async()=>{const keys=await caches.keys();await Promise.all(keys.filter(key=>CACHE_PREFIXES.some(prefix=>key.startsWith(prefix))&&key!==CACHE).map(key=>caches.delete(key)));})()));
...
self.addEventListener('fetch',e=>{const request=e.request;const u=new URL(request.url);const scopePath=new URL('./',self.location.href).pathname;if(isSecurityCriticalRequest(u,scopePath)){e.respondWith(networkOnlyNoStore(request));return;}e.respondWith(cacheFirst(request))});`;
  ...
  // ACT-N4: URL-equivalent critical precache forms and Cache API fallback in the network-only sink must fail closed.
  const dotSegmentSw = goodSw.replace("const PRECACHE=['/index.html'];", "const PRECACHE=['/index.html','/foo/../app-guard.js'];\nself.addEventListener('install',e=>e.waitUntil(caches.open('x').then(cache=>cache.addAll(PRECACHE))));");
  await writeFile(path.join(t, 'sw-act-n4-dot-segment.js'), dotSegmentSw);
  r = run('check-sw-security-freeze.mjs', [path.join(t, 'sw-act-n4-dot-segment.js'), swDir, goodCriticalListPath]);
  expect('NC-ACT-N4-dot-segment-critical-precache-fails', r.status !== 0 && (r.stderr.includes('CRITICAL') || r.stdout.includes('CRITICAL')));
  const doubleSlashSw = goodSw.replace("const PRECACHE=['/index.html'];", "const PRECACHE=['/index.html','/foo//../app-guard.js'];\nself.addEventListener('install',e=>e.waitUntil(caches.open('x').then(cache=>cache.addAll(PRECACHE))));");
  await writeFile(path.join(t, 'sw-act-n4-double-slash.js'), doubleSlashSw);
  r = run('check-sw-security-freeze.mjs', [path.join(t, 'sw-act-n4-double-slash.js'), swDir, goodCriticalListPath]);
  expect('NC-ACT-N4-double-slash-critical-precache-fails', r.status !== 0 && (r.stderr.includes('CRITICAL') || r.stdout.includes('CRITICAL')));
  const sinkCacheFallbackSw = goodSw.replace("async function networkOnlyNoStore(request){return fetch(request,{cache:'no-store'})}", "async function networkOnlyNoStore(request){try{return await fetch(request,{cache:'no-store'})}catch{return caches.match(request)}}");
  await writeFile(path.join(t, 'sw-act-n4-cache-read-fallback.js'), sinkCacheFallbackSw);
  r = run('check-sw-security-freeze.mjs', [path.join(t, 'sw-act-n4-cache-read-fallback.js'), swDir, goodCriticalListPath]);
  expect('NC-ACT-N4-network-only-cache-read-fallback-fails', r.status !== 0 && (r.stderr.includes('Cache API access') || r.stdout.includes('Cache API access') || r.stderr.includes('CRITICAL') || r.stdout.includes('CRITICAL')));
  ...
  // LU-N2/P4 hardening: iterator variables are lexical call-site bindings, not global names.
  // These three cases reproduce the LUDUS OPTIONAL_ASSETS blind spot from independent review.
  const p4PrefixSameName = `const GHRAB_PLATFORM_P3_ASSETS=['/index.html'];
const OPTIONAL_ASSETS=['/app-guard.js'];
self.addEventListener('install',e=>e.waitUntil((async()=>{const cache=await caches.open('x');await Promise.all(GHRAB_PLATFORM_P3_ASSETS.map(asset=>cache.add(asset)));await Promise.allSettled(OPTIONAL_ASSETS.map((asset)=>cache.add(asset)));})()));
`;
  await writeFile(path.join(t, 'sw-p4-name-collision.js'), p4PrefixSameName + goodSw);
  r = run('check-sw-security-freeze.mjs', [path.join(t, 'sw-p4-name-collision.js'), swDir, goodCriticalListPath]);
  expect('NC-LU-N2-P4-parenthesized-name-collision-fails', r.status !== 0 && (r.stderr.includes('CRITICAL') || r.stdout.includes('CRITICAL') || r.stderr.includes('AMBER') || r.stdout.includes('AMBER')));

  const p4PrefixNoCollision = `const GHRAB_PLATFORM_P3_ASSETS=['/index.html'];
const OPTIONAL_ASSETS=['/app-guard.js'];
self.addEventListener('install',e=>e.waitUntil((async()=>{const cache=await caches.open('x');await Promise.all(GHRAB_PLATFORM_P3_ASSETS.map(a=>cache.add(a)));await Promise.allSettled(OPTIONAL_ASSETS.map((asset)=>cache.add(asset)));})()));
`;
  await writeFile(path.join(t, 'sw-p4b-parenthesized-no-collision.js'), p4PrefixNoCollision + goodSw);
  r = run('check-sw-security-freeze.mjs', [path.join(t, 'sw-p4b-parenthesized-no-collision.js'), swDir, goodCriticalListPath]);
  expect('NC-LU-N2-P4b-parenthesized-no-collision-fails-or-amber', r.status !== 0 && (r.stderr.includes('CRITICAL') || r.stdout.includes('CRITICAL') || r.stderr.includes('AMBER') || r.stdout.includes('AMBER')));

  const p4PrefixBareParam = `const GHRAB_PLATFORM_P3_ASSETS=['/index.html'];
const OPTIONAL_ASSETS=['/app-guard.js'];
self.addEventListener('install',e=>e.waitUntil((async()=>{const cache=await caches.open('x');await Promise.all(GHRAB_PLATFORM_P3_ASSETS.map(asset=>cache.add(asset)));await Promise.allSettled(OPTIONAL_ASSETS.map(asset=>cache.add(asset)));})()));
`;
  await writeFile(path.join(t, 'sw-p4c-bare-param.js'), p4PrefixBareParam + goodSw);
  r = run('check-sw-security-freeze.mjs', [path.join(t, 'sw-p4c-bare-param.js'), swDir, goodCriticalListPath]);
  expect('NC-LU-N2-P4c-bare-param-fails', r.status !== 0 && (r.stderr.includes('CRITICAL') || r.stdout.includes('CRITICAL')));
...
self.addEventListener('fetch',event=>{const request=event.request;const url=new URL(request.url);const scopePath=new URL('./',self.location.href).pathname;if(isSecurityCriticalRequest(url,scopePath)){event.respondWith(networkOnlyNoStore(request));return;}event.respondWith(cacheFirst(request));});`;
  ...
  // ACT-N7: reachable helper/alias cache reads, fallback fetch without no-store, and missing activate cleanup must fail closed.
  const helperCacheReadSw = goodSw.replace("async function networkOnlyNoStore(request){return fetch(request,{cache:'no-store'})}", "async function lastKnownGood(request){return caches.match(request)}\nasync function networkOnlyNoStore(request){try{return await fetch(request,{cache:'no-store'})}catch{return lastKnownGood(request)}}");
  await writeFile(path.join(t,'sw-act-n7-helper-cache-read.js'),helperCacheReadSw); r=run('check-sw-security-freeze.mjs',[path.join(t,'sw-act-n7-helper-cache-read.js'),swDir,goodCriticalListPath]);
  expect('NC-ACT-N7-helper-cache-read-fails',r.status!==0&&((r.stderr+r.stdout).includes('call graph')||(r.stderr+r.stdout).includes('CRITICAL')));
  const aliasCacheReadSw = goodSw.replace("async function networkOnlyNoStore(request){return fetch(request,{cache:'no-store'})}", "async function networkOnlyNoStore(request){const c=caches;try{return await fetch(request,{cache:'no-store'})}catch{return c.match(request)}}");
  await writeFile(path.join(t,'sw-act-n7-alias-cache-read.js'),aliasCacheReadSw); r=run('check-sw-security-freeze.mjs',[path.join(t,'sw-act-n7-alias-cache-read.js'),swDir,goodCriticalListPath]);
  expect('NC-ACT-N7-alias-cache-read-fails',r.status!==0&&((r.stderr+r.stdout).includes('call graph')||(r.stderr+r.stdout).includes('CRITICAL')));
  const fallbackFetchSw = goodSw.replace("async function networkOnlyNoStore(request){return fetch(request,{cache:'no-store'})}", "async function networkOnlyNoStore(request){try{return await fetch(request,{cache:'no-store'})}catch{return fetch(request)}}");
  await writeFile(path.join(t,'sw-act-n7-fetch-without-no-store.js'),fallbackFetchSw); r=run('check-sw-security-freeze.mjs',[path.join(t,'sw-act-n7-fetch-without-no-store.js'),swDir,goodCriticalListPath]);
  expect('NC-ACT-N7-fetch-without-no-store-fails',r.status!==0&&((r.stderr+r.stdout).includes('no-store')||(r.stderr+r.stdout).includes('MEDIUM')));
  const noCleanupSw = goodSw.replace("keys.filter(key=>CACHE_PREFIXES.some(prefix=>key.startsWith(prefix))&&key!==CACHE).map(key=>caches.delete(key))", "keys.map(key=>Promise.resolve(false))");
  await writeFile(path.join(t,'sw-act-n7-activate-cleanup-removed.js'),noCleanupSw); r=run('check-sw-security-freeze.mjs',[path.join(t,'sw-act-n7-activate-cleanup-removed.js'),swDir,goodCriticalListPath]);
  expect('NC-ACT-N7-activate-cleanup-removed-fails',r.status!==0&&((r.stderr+r.stdout).includes('activate handler')||(r.stderr+r.stdout).includes('HIGH')));

  // ACT-N11: module-scope aliases and semantically neutered cleanup must fail by reachability/effect.
  const moduleAliasSw = goodSw.replace("async function networkOnlyNoStore(request){return fetch(request,{cache:'no-store'})}", "const cacheApi=caches;\nasync function networkOnlyNoStore(request){try{return await fetch(request,{cache:'no-store'})}catch{return cacheApi.match(request)}}");
  await writeFile(path.join(t,'sw-act-n11-module-alias-cache-read.js'),moduleAliasSw); r=run('check-sw-security-freeze.mjs',[path.join(t,'sw-act-n11-module-alias-cache-read.js'),swDir,goodCriticalListPath]);
  expect('NC-ACT-N11-module-alias-cache-read-fails',r.status!==0&&((r.stderr+r.stdout).includes('Cache Storage')||(r.stderr+r.stdout).includes('CRITICAL')));
  const objectAliasSw = goodSw.replace("async function networkOnlyNoStore(request){return fetch(request,{cache:'no-store'})}", "const store={c:caches};\nasync function networkOnlyNoStore(request){try{return await fetch(request,{cache:'no-store'})}catch{return store.c.match(request)}}");
  await writeFile(path.join(t,'sw-act-n11-object-alias-cache-read.js'),objectAliasSw); r=run('check-sw-security-freeze.mjs',[path.join(t,'sw-act-n11-object-alias-cache-read.js'),swDir,goodCriticalListPath]);
  expect('NC-ACT-N11-object-alias-cache-read-fails',r.status!==0&&((r.stderr+r.stdout).includes('Cache Storage')||(r.stderr+r.stdout).includes('CRITICAL')));
  const neuteredCleanupSw = goodSw.replace("CACHE_PREFIXES.some(prefix=>key.startsWith(prefix))&&key!==CACHE", "CACHE_PREFIXES.some(prefix=>key.startsWith(prefix))&&key!==CACHE&&false");
  await writeFile(path.join(t,'sw-act-n11-activate-cleanup-neutered.js'),neuteredCleanupSw); r=run('check-sw-security-freeze.mjs',[path.join(t,'sw-act-n11-activate-cleanup-neutered.js'),swDir,goodCriticalListPath]);
  expect('NC-ACT-N11-activate-cleanup-neutered-fails',r.status!==0&&((r.stderr+r.stdout).includes('activate handler')||(r.stderr+r.stdout).includes('HIGH')));

  // ACT-N16/N17: dynamic/reflexive CacheStorage identity and manufactured-success
  // network fallbacks must not bypass the whole-module behavioral verifier.
  const dynamicCachePropertySw = goodSw.replace("async function networkOnlyNoStore(request){return fetch(request,{cache:'no-store'})}", "async function networkOnlyNoStore(request){try{return await fetch(request,{cache:'no-store'})}catch{return self['ca'+'ches'].match(request)}}");
  await writeFile(path.join(t,'sw-act-n16-dynamic-cache-property.js'),dynamicCachePropertySw); r=run('check-sw-security-freeze.mjs',[path.join(t,'sw-act-n16-dynamic-cache-property.js'),swDir,goodCriticalListPath]);
  expect('NC-ACT-N16-dynamic-cache-property-fails',r.status!==0&&((r.stderr+r.stdout).includes('Cache Storage')||(r.stderr+r.stdout).includes('CRITICAL')));
  const reflectCachePropertySw = goodSw.replace("async function networkOnlyNoStore(request){return fetch(request,{cache:'no-store'})}", "async function networkOnlyNoStore(request){try{return await fetch(request,{cache:'no-store'})}catch{return Reflect.get(self,'caches').match(request)}}");
  await writeFile(path.join(t,'sw-act-n16-reflect-cache-property.js'),reflectCachePropertySw); r=run('check-sw-security-freeze.mjs',[path.join(t,'sw-act-n16-reflect-cache-property.js'),swDir,goodCriticalListPath]);
  expect('NC-ACT-N16-reflect-cache-property-fails',r.status!==0&&((r.stderr+r.stdout).includes('Cache Storage')||(r.stderr+r.stdout).includes('CRITICAL')));
  const manufacturedSuccessSw = goodSw.replace("async function networkOnlyNoStore(request){return fetch(request,{cache:'no-store'})}", "async function networkOnlyNoStore(request){try{return await fetch(request,{cache:'no-store'})}catch{return new Response('synthetic',{status:200})}}");
  await writeFile(path.join(t,'sw-act-n17-manufactured-success.js'),manufacturedSuccessSw); r=run('check-sw-security-freeze.mjs',[path.join(t,'sw-act-n17-manufactured-success.js'),swDir,goodCriticalListPath]);
  expect('NC-ACT-N17-manufactured-success-fails',r.status!==0&&((r.stderr+r.stdout).includes('networkOnlyNoStore')||(r.stderr+r.stdout).includes('MEDIUM')));
  const explicitFailureResponseSw = goodSw.replace("async function networkOnlyNoStore(request){return fetch(request,{cache:'no-store'})}", "async function networkOnlyNoStore(request){try{return await fetch(request,{cache:'no-store'})}catch{return new Response('offline',{status:503})}}");
  await writeFile(path.join(t,'sw-act-n17-explicit-failure-response.js'),explicitFailureResponseSw); r=run('check-sw-security-freeze.mjs',[path.join(t,'sw-act-n17-explicit-failure-response.js'),swDir,goodCriticalListPath]);
  expect('ACT-N17-explicit-unsuccessful-response-is-fail-closed',r.status===0);
...
`;
  ...
  await mkdir(path.join(aiRoot,'engines'),{recursive:true});
  await writeFile(path.join(aiRoot,'engines','provider.html'), '<script>fetch("https://generativelanguage.googleapis.com/v1beta/models/x:generateContent")</script>');
  r = run('verify-ai-assurance-fingerprint.mjs', [path.join(aiRoot, 'fingerprint.json'), path.join(aiRoot, 'inventory.json'), aiRoot]);
  expect('NC-LU-N17-G5-engine-provider-endpoint-drift-fails', r.status !== 0 && (r.stderr.includes('inventory-drift-untracked') || r.stdout.includes('inventory-drift-untracked'))); await rm(path.join(aiRoot,'engines','provider.html'));
  await mkdir(path.join(aiRoot,'runtime'),{recursive:true});
  await writeFile(path.join(aiRoot,'runtime','provider.js'), 'export const op="generateContent";');
  r = run('verify-ai-assurance-fingerprint.mjs', [path.join(aiRoot, 'fingerprint.json'), path.join(aiRoot, 'inventory.json'), aiRoot]);
  expect('NC-LU-N17-runtime-provider-token-drift-fails', r.status !== 0); await rm(path.join(aiRoot,'runtime','provider.js'));
  await mkdir(path.join(aiRoot,'public'),{recursive:true});
  await writeFile(path.join(aiRoot,'public','provider.js'), 'fetch("https://api.openai.com/v1/responses")');
  r = run('verify-ai-assurance-fingerprint.mjs', [path.join(aiRoot, 'fingerprint.json'), path.join(aiRoot, 'inventory.json'), aiRoot]);
  expect('NC-LU-N17-G7-public-root-provider-drift-fails', r.status !== 0); await rm(path.join(aiRoot,'public','provider.js'));
  await mkdir(path.join(aiRoot,'scripts'),{recursive:true});
  await writeFile(path.join(aiRoot,'scripts','provider.mjs'), 'const endpoint="https://api.anthropic.com/v1/messages";');
  r = run('verify-ai-assurance-fingerprint.mjs', [path.join(aiRoot, 'fingerprint.json'), path.join(aiRoot, 'inventory.json'), aiRoot]);
  expect('NC-LU-N17-scripts-provider-endpoint-drift-fails', r.status !== 0); await rm(path.join(aiRoot,'scripts','provider.mjs'));

  await mkdir(path.join(aiRoot,'vendor','platform'),{recursive:true});
  await writeFile(path.join(aiRoot,'vendor','platform','provider.js'), 'fetch("https://api.openai.com/v1/responses")');
  r = run('verify-ai-assurance-fingerprint.mjs', [path.join(aiRoot, 'fingerprint.json'), path.join(aiRoot, 'inventory.json'), aiRoot]);
  expect('NC-ACT-N5-vendor-provider-endpoint-drift-fails', r.status !== 0 && (r.stderr.includes('inventory-drift-untracked') || r.stdout.includes('inventory-drift-untracked'))); await rm(path.join(aiRoot,'vendor'),{recursive:true,force:true});
  await mkdir(path.join(aiRoot,'qa'),{recursive:true});
  await writeFile(path.join(aiRoot,'qa','provider.mjs'), 'const endpoint="https://api.anthropic.com/v1/messages";');
  r = run('verify-ai-assurance-fingerprint.mjs', [path.join(aiRoot, 'fingerprint.json'), path.join(aiRoot, 'inventory.json'), aiRoot]);
  expect('NC-ACT-N5-qa-provider-endpoint-drift-fails', r.status !== 0 && (r.stderr.includes('inventory-drift-untracked') || r.stdout.includes('inventory-drift-untracked'))); await rm(path.join(aiRoot,'qa'),{recursive:true,force:true});
  await mkdir(path.join(aiRoot,'.github','workflows'),{recursive:true}); await writeFile(path.join(aiRoot,'.github','workflows','ai.yml'),'env: { ENDPOINT: https://api.openai.com/v1/responses }\n');
  r=run('verify-ai-assurance-fingerprint.mjs',[path.join(aiRoot,'fingerprint.json'),path.join(aiRoot,'inventory.json'),aiRoot]); expect('NC-ACT-N9-workflow-provider-drift-fails',r.status!==0); await rm(path.join(aiRoot,'.github'),{recursive:true,force:true});
  await writeFile(path.join(aiRoot,'provider-root.mjs'),'const endpoint="https://api.anthropic.com/v1/messages";\n'); r=run('verify-ai-assurance-fingerprint.mjs',[path.join(aiRoot,'fingerprint.json'),path.join(aiRoot,'inventory.json'),aiRoot]); expect('NC-ACT-N9-root-provider-drift-fails',r.status!==0); await rm(path.join(aiRoot,'provider-root.mjs'));
  await mkdir(path.join(aiRoot,'security','garp25','tools'),{recursive:true}); await writeFile(path.join(aiRoot,'security','garp25','tools','provider.mjs'),'fetch("https://api.openai.com/v1/responses")\n'); r=run('verify-ai-assurance-fingerprint.mjs',[path.join(aiRoot,'fingerprint.json'),path.join(aiRoot,'inventory.json'),aiRoot]); expect('NC-ACT-N9-security-tool-provider-drift-fails',r.status!==0); await rm(path.join(aiRoot,'security'),{recursive:true,force:true});
  await mkdir(path.join(aiRoot,'docs'),{recursive:true}); await writeFile(path.join(aiRoot,'docs','provider.md'),'https://api.anthropic.com/v1/messages\n'); r=run('verify-ai-assurance-fingerprint.mjs',[path.join(aiRoot,'fingerprint.json'),path.join(aiRoot,'inventory.json'),aiRoot]); expect('NC-ACT-N9-docs-provider-drift-fails',r.status!==0); await rm(path.join(aiRoot,'docs'),{recursive:true,force:true});
  await mkdir(path.join(aiRoot,'.github','scripts'),{recursive:true}); await writeFile(path.join(aiRoot,'.github','scripts','provider.mjs'),'fetch("https://api.openai.com/v1/responses")\n'); r=run('verify-ai-assurance-fingerprint.mjs',[path.join(aiRoot,'fingerprint.json'),path.join(aiRoot,'inventory.json'),aiRoot]); expect('NC-ACT-N14-github-scripts-provider-drift-fails',r.status!==0); await rm(path.join(aiRoot,'.github'),{recursive:true,force:true});
  await mkdir(path.join(aiRoot,'release'),{recursive:true}); await writeFile(path.join(aiRoot,'release','provider.mjs'),'fetch("https://api.openai.com/v1/responses")\n'); r=run('verify-ai-assurance-fingerprint.mjs',[path.join(aiRoot,'fingerprint.json'),path.join(aiRoot,'inventory.json'),aiRoot]); expect('NC-ACT-N14-new-root-dir-provider-drift-fails',r.status!==0); await rm(path.join(aiRoot,'release'),{recursive:true,force:true});
  await writeFile(path.join(aiRoot,'provider-hook'),'https://api.anthropic.com/v1/messages\n'); r=run('verify-ai-assurance-fingerprint.mjs',[path.join(aiRoot,'fingerprint.json'),path.join(aiRoot,'inventory.json'),aiRoot]); expect('NC-ACT-N14-extensionless-root-provider-drift-fails',r.status!==0); await rm(path.join(aiRoot,'provider-hook'));
  ...
  const linkedRi = JSON.parse(await readFile(path.join(t, 'ri.json'), 'utf8'));
  linkedRi.sourceCommit = good.source.revision;
  linkedRi.buildProvenanceSha256 = createHash('sha256').update(await readFile(path.join(t, 'prov.json'))).digest('hex');
  linkedRi.evidenceManifestSha256 = createHash('sha256').update(await readFile(path.join(t, 'evidence.json'))).digest('hex');
  await writeFile(path.join(t, 'ri.json'), JSON.stringify(linkedRi, null, 2) + '\n');
