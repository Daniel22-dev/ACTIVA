...
const norm = value => {
  const raw = String(value || '').trim().replace(/^['"`]|['"`]$/g, '').replaceAll('\\', '/');
  if (!raw) return '';
  try {
    const url = new URL(raw, 'https://garp.invalid/');
    ...
    return raw.replace(/[?#].*$/, '').replace(/^\.\//, '').replace(/^\/+/, '').replace(/\/{2,}/g, '/');
  }
};
const matchesCritical = value => { const v=norm(value).toLowerCase(); if (!v) return false; return critical.some(c => { const cc=norm(c).toLowerCase(); return cc && (v.includes(cc) || cc.includes(v)); }); };
...
for (const p of precached) {
  for (const c of critical) {
    const pp=norm(p).toLowerCase(), cc=norm(c).toLowerCase();
...
function namedFunctionMap(){
  const out=new Map();
  for(const m of sw.matchAll(/(?:async\s+)?function\s+([A-Za-z_$][\w$]*)\s*\([^)]*\)\s*\{/g)){
    const open=m.index+m[0].lastIndexOf('{'), end=scanBalanced(sw,open,'{','}');
    if(end>open) out.set(m[1],{source:sw.slice(m.index,end+1),body:sw.slice(open+1,end)});
  }
  return out;
}
function parseFetchCalls(body, owner){
  const calls=[];
  for(const m of body.matchAll(/\bfetch\s*\(/g)){
    const open=m.index+m[0].lastIndexOf('('), end=scanBalanced(body,open); if(end<0){calls.push({owner,pass:false,reason:'unbalanced'});continue;}
    const args=splitTopLevel(body.slice(open+1,end)); const opts=args[1]||'';
    calls.push({owner,args:args.length,pass:/\bcache\s*:\s*['"]no-store['"]/.test(opts),options:opts.slice(0,180)});
  }
  return calls;
}
function analyzeReachableNetworkOnlySink(){
  const funcs=namedFunctionMap(), queue=['networkOnlyNoStore'], seen=new Set(), cacheAccesses=[], fetchCalls=[];
  while(queue.length){
    const name=queue.shift(); if(seen.has(name))continue; seen.add(name); const f=funcs.get(name); if(!f)continue; const body=f.body;
    if(/\bcaches\s*(?:\.|\[)/.test(body)) cacheAccesses.push({function:name,kind:'direct-caches'});
    const aliases=[...body.matchAll(/\b(?:const|let|var)\s+([A-Za-z_$][\w$]*)\s*=\s*caches\b/g)].map(m=>m[1]);
    for(const a of aliases){const are=new RegExp(`\\b${a}\\s*(?:\\.|\\[)`); if(are.test(body)) cacheAccesses.push({function:name,kind:'cache-alias',alias:a});}
    fetchCalls.push(...parseFetchCalls(body,name));
    for(const c of body.matchAll(/\b([A-Za-z_$][\w$]*)\s*\(/g)){const callee=c[1]; if(funcs.has(callee)&&callee!==name)queue.push(callee);}
  }
  const allFetchNoStore=fetchCalls.length>0&&fetchCalls.every(x=>x.pass);
  return {status:cacheAccesses.length===0&&allFetchNoStore?'PASS':'FAIL',reachableFunctions:[...seen],cacheAccesses,fetchCalls,allFetchNoStore};
}
function analyzeActivateCleanup(){
  const m=/addEventListener\s*\(\s*['"]activate['"]\s*,/.exec(sw); if(!m)return {status:'FAIL',reason:'activate-handler-missing'};
  const open=sw.indexOf('(',m.index), end=scanBalanced(sw,open); if(end<0)return {status:'FAIL',reason:'activate-handler-unbalanced'};
  const body=sw.slice(open+1,end);
  const checks={
    keys:/\bcaches\s*\.\s*keys\s*\(/.test(body),
    prefixes:/\bCACHE_PREFIXES\b/.test(body)&&/\.startsWith\s*\(\s*prefix\s*\)/.test(body),
    keepsCurrent:/\bkey\s*!==\s*CACHE\b|\bCACHE\s*!==\s*key\b/.test(body),
    deletes:/\bcaches\s*\.\s*delete\s*\(\s*key\s*\)/.test(body)
  };
  return {status:Object.values(checks).every(Boolean)?'PASS':'FAIL',checks};
}
async function behavioralActivateCleanupGuard() {
  // ACT-N11: prove cleanup by effect, not syntax. The real handler must delete
  // stale cache keys belonging to every declared application prefix, while
  // preserving the current cache and unrelated caches.
  const current = resolveExpr('CACHE')?.[0] || '';
  const prefixes = resolveExpr('CACHE_PREFIXES') || [];
  if (!current || !prefixes.length) return { status:'FAIL', reason:'cache-name-or-prefixes-unresolved', current, prefixes };
  const stale = [...new Set(prefixes.map((prefix, i) => `${prefix}__garp_stale_${i}__`).filter(k => k !== current))];
  const unrelated = '__garp_unrelated_cache__';
  const keys = [current, ...stale, unrelated];
  const deletes = [], registrations = [], waiters = [];
  const cachesMock = Object.freeze({
    keys(){ return Promise.resolve([...keys]); },
    delete(key){ deletes.push(String(key)); return Promise.resolve(true); },
    open(){ return Promise.resolve(Object.freeze({ addAll(){return Promise.resolve();}, add(){return Promise.resolve();}, put(){return Promise.resolve();}, match(){return Promise.resolve(null);} })); },
  ...
  const selfMock = {
    ...
    clients: Object.freeze({ claim(){ return Promise.resolve(true); } }),
    addEventListener(type, handler){ registrations.push({type:String(type), handler}); },
    skipWaiting(){ return undefined; }
  };
  const context=createContext({
    self:selfMock, URL, caches:cachesMock,
    console:Object.freeze({log(){},warn(){},error(){}}),
    fetch(){ return Promise.reject(new Error('not-used')); }
  });
  let error=null;
  try { new Script(sw).runInContext(context,{timeout:75}); }
  catch(e){ return {status:'FAIL',reason:'service-worker-module-eval-error',error:String(e),current,prefixes,stale,deletes}; }
  const handlers=registrations.filter(r=>r.type==='activate'&&typeof r.handler==='function');
  if(handlers.length!==1) return {status:'FAIL',reason:'activate-handler-registration-ambiguous',handlers:handlers.length,current,prefixes,stale,deletes};
  const event=Object.freeze({waitUntil(value){waiters.push(Promise.resolve(value));}});
  try { handlers[0].handler(event); await Promise.all(waiters); }
  catch(e){ error=String(e); }
  const deleted=new Set(deletes);
  const missingStale=stale.filter(k=>!deleted.has(k));
  const deletedCurrent=deleted.has(current);
  const deletedUnrelated=deleted.has(unrelated);
  const pass=!error&&waiters.length>0&&missingStale.length===0&&!deletedCurrent&&!deletedUnrelated;
  return {status:pass?'PASS':'FAIL',error,current,prefixes,stale,deletes,missingStale,deletedCurrent,deletedUnrelated,waitUntilCalls:waiters.length};
}
...
async function behavioralNetworkOnlyNoStoreGuard() {
  // ACT-N11: evaluate the complete service-worker module so module-scope aliases
  // (const cacheApi=caches, object aliases, destructured references, helpers) are
  // present exactly as they are at runtime. Exercise both the successful network
  // path and the failure path; a critical network-only sink must never touch Cache
  // Storage and the failure path must remain fail-closed.
  const request = Object.freeze({ url: 'https://example.invalid/__ghrab_scope__/runtime-config.js', method: 'GET' });
  async function runScenario(fetchFails) {
    const fetchCalls = [], cacheCalls = [], registrations = [];
    const cache = Object.freeze({
      put(...args){ cacheCalls.push({ method: 'put', argc: args.length }); return Promise.resolve(); },
      add(...args){ cacheCalls.push({ method: 'add', argc: args.length }); return Promise.resolve(); },
      addAll(...args){ cacheCalls.push({ method: 'addAll', argc: args.length }); return Promise.resolve(); },
      match(...args){ cacheCalls.push({ method: 'match', argc: args.length }); return Promise.resolve(null); }
    });
    const cachesMock = Object.freeze({
      open(...args){ cacheCalls.push({ method: 'open', argc: args.length }); return Promise.resolve(cache); },
      match(...args){ cacheCalls.push({ method: 'match', argc: args.length }); return Promise.resolve(null); },
      delete(...args){ cacheCalls.push({ method: 'delete', argc: args.length }); return Promise.resolve(false); },
      keys(...args){ cacheCalls.push({ method: 'keys', argc: args.length }); return Promise.resolve([]); }
    });
    class SyntheticResponse {
      ...
      clone(){ return new SyntheticResponse(this.body, { status: this.status, statusText: this.statusText, headers: this.headers }); }
      async text(){ return String(this.body ?? ''); }
      async json(){ return JSON.parse(String(this.body ?? 'null')); }
      static json(value, init = {}) { return new SyntheticResponse(JSON.stringify(value), { ...init, headers: { 'content-type':'application/json', ...(init.headers || {}) } }); }
    }
    const selfMock = {
      location: Object.freeze({ origin: 'https://example.invalid', href: 'https://example.invalid/__ghrab_scope__/sw.js' }),
      registration: Object.freeze({ scope: 'https://example.invalid/__ghrab_scope__/' }),
    ...
    const context = createContext({
      ...
      caches: cachesMock,
      fetch: (req, options) => {
        fetchCalls.push({ sameRequest: req === request, options: options ? { cache: options.cache } : null });
        if (fetchFails) throw new Error('synthetic-network-failure');
        return Object.freeze({ ok: true, __ghrabFetch: true });
      }
    });
    let moduleError = null;
    try { new Script(`${sw}\n;this.__ghrabNetworkOnlyNoStore = networkOnlyNoStore;`).runInContext(context, { timeout: 75 }); }
    catch (e) { moduleError = String(e); }
    if (moduleError) return { fetchFails, status: 'FAIL', reason: 'service-worker-module-eval-error', error: moduleError, fetchCalls, cacheCalls, registrations: registrations.map(r=>r.type) };
    const fn = context.__ghrabNetworkOnlyNoStore;
    if (typeof fn !== 'function') return { fetchFails, status: 'FAIL', reason: 'network-only-sink-not-callable', fetchCalls, cacheCalls, registrations: registrations.map(r=>r.type) };
    let result, error = null;
    try { result = await fn(request); } catch (e) { error = String(e); }
    const allFetchNoStore = fetchCalls.length > 0 && fetchCalls.every(c => c.sameRequest && c.options?.cache === 'no-store');
    const cacheFree = cacheCalls.length === 0;
    const unsuccessfulResponse = !error && result && typeof result === 'object' && result.ok === false;
    const failClosed = fetchFails ? (!!error || unsuccessfulResponse) : !error;
    const pass = allFetchNoStore && cacheFree && failClosed && (fetchFails || fetchCalls.length === 1);
    return {
      fetchFails, status: pass ? 'PASS' : 'FAIL', error, fetchCalls, cacheCalls,
      allFetchNoStore, cacheFree, failClosed, unsuccessfulResponse,
      resultObserved: result !== undefined,
      resultStatus: result && typeof result === 'object' && 'status' in result ? result.status : null,
      resultOk: result && typeof result === 'object' && 'ok' in result ? result.ok : null,
      registrations: registrations.map(r=>r.type)
    };
  }
  const success = await runScenario(false);
  const failure = await runScenario(true);
  return {
    status: success.status === 'PASS' && failure.status === 'PASS' ? 'PASS' : 'FAIL',
    success,
    failure,
    cacheCalls: [...(success.cacheCalls||[]), ...(failure.cacheCalls||[])],
    fetchCalls: [...(success.fetchCalls||[]), ...(failure.fetchCalls||[])],
    checkerMode: 'whole-module-success-and-failure-reachability'
  };
}
const networkOnlyCallGraph = analyzeReachableNetworkOnlySink();
const activateCleanupStructural = analyzeActivateCleanup();
const activateCleanupBehavioral = await behavioralActivateCleanupGuard();
const activateCleanup = { status: activateCleanupStructural.status === 'PASS' && activateCleanupBehavioral.status === 'PASS' ? 'PASS' : 'FAIL', structural: activateCleanupStructural, behavioral: activateCleanupBehavioral };
if (networkOnlyCallGraph.cacheAccesses.length) findings.push({ severity: 'CRITICAL', issue: 'networkOnlyNoStore call graph obsahuje Cache API access', detail: networkOnlyCallGraph });
if (!networkOnlyCallGraph.allFetchNoStore) findings.push({ severity: 'MEDIUM', issue: 'networkOnlyNoStore call graph obsahuje fetch bez cache:no-store nebo zadny prokazany fetch', detail: networkOnlyCallGraph });
if (activateCleanup.status !== 'PASS') findings.push({ severity: 'HIGH', issue: 'activate handler neprokazuje odstraneni starych cache vlastni aplikace', detail: activateCleanup });
...
const networkOnlySinkCacheAccess = [...networkOnlySinkBody.matchAll(/\bcaches\s*(?:\.|\[)/g)].map(m => m[0]);
...
if (networkOnlySinkCacheAccess.length) findings.push({
  severity: 'CRITICAL', issue: 'networkOnlyNoStore obsahuje Cache API access; network-only sink nesmi cist ani zapisovat Cache Storage', detail: networkOnlySinkCacheAccess
});
const networkOnlySink = await behavioralNetworkOnlyNoStoreGuard();
if ((networkOnlySink.cacheCalls || []).length) findings.push({
  severity: 'CRITICAL', issue: 'networkOnlyNoStore dosazitelne pristupuje ke Cache Storage; network-only sink musi zustat cache-free i pri chybe site', detail: networkOnlySink
});
...
const out = {
  ...
  fetchRouteBehavior,
  networkOnlySink,
  networkOnlyCallGraph,
  activateCleanup,
  criticalAssetsInDeployment: criticalFiles,
  authoritativeCriticalAssets: critical,
  findings,
  checkerRevision: 'garp-2.5.1-act-n16-n17-value-failclosed-hotfix',
  note: 'Static bounded Cache API analysis plus bounded behavioral evaluation of service-worker registration, isSecurityCriticalRequest, the complete unique fetch handler, and networkOnlyNoStore. PASS requires exactly one dynamically observed fetch handler registration; no pre-guard routing-sensitive read or network/cache/respondWith side effect; after a positive guard no Request/URL/FetchEvent read except event.respondWith; exactly one total routing/network/cache effect (networkOnlyNoStore); and a sink call graph whose successful and failing runtime paths, evaluated with the complete module scope (including self.caches/global CacheStorage identity and dynamic/reflexive property access), contain no reachable Cache Storage access, whose fetch calls all use cache:no-store, and whose synthetic network-failure path rejects or returns an explicitly unsuccessful response; activate must prove stale-cache cleanup by observed caches.delete effects for every app cache prefix while preserving the current and unrelated caches. Does not replace SIM-07 or live revocation tests.'
};
